...
logmod1 <- glm(children_intention ~ sex + province + pop_center 
             ...
             data = dat_new, family = "binomial") #fit model 1 into a logmod1
summary(logmod1)
anova(logmod1)
```
...
logmod2 <- glm(children_intention ~ sex + marital_status + education 
               ...
               + fin_supp_other, data = dat_new, family = "binomial") #fit model 2 into a logmod2
summary(logmod2)
anova(logmod2)
```
...
logmod4 <- glm(children_intention ~ sex + marital_status + own_rent 
               ...
               family = "binomial") #added sex, own_rent, hh_size, partner_main_activity and removed partner_education
summary(logmod4)
anova(logmod4)
```
...
logmod5 <- glm(children_intention ~ sex + marital_status + education + own_rent
               ...
               data = dat_new, family = "binomial") #added education and partner education
summary(logmod5)
anova(logmod5)
```

logmod5 is the best cause smallest AIC compared to logmod 3 and 4 and has many significant variables unlike logmod1 and logmod2
  so we'll do ggplot on logmod5
